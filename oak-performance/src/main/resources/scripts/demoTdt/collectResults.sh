...
if [ "$CURRENT_NODE" == "$MONGOS_MAIN_PLATFORM" ]; then
    ...
    do
        VALUE=`mongo --host localhost $DATABASE_NAME --port $MONGOS_PORT --quiet --eval "var result=db.${COLLECTION_NAME}.find({'commitNumber':\"$c\"},{v:1,_id:0}).sort({clusterNodeId:-1}).toArray();printjson(result)"`
    ...
    VALUE=`mongo --host localhost $DATABASE_NAME --port $MONGOS_PORT --quiet --eval "var result=db.${SUMMARY_CNAME}.find({},{_id:0}).toArray();printjson(result)"`
    echo $VALUE > $SUMMARY
    VALUE=`mongo --host localhost $DATABASE_NAME --port $MONGOS_PORT --quiet --eval "var result=db.stats();printjson(result)"`
